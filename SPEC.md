...
Build a small, embeddable AI chatbot for a client website that answers visitor questions using a single PDF as its knowledge source (a basic RAG setup), streams its replies, logs conversations, and can be deployed cheaply on the client's hosting. This is a fixed-scope MVP, not an enterprise system: the priority is a working, maintainable v1 over completeness.

The MVP must run end to end with or without a client PDF. When no PDF has been ingested, the backend seeds a small bundled demo corpus and answers from it, so the full RAG pipeline (embed, retrieve, ground, stream) is demonstrably working on a fresh checkout. Ingesting the real client PDF later replaces the demo content.

## 2. Scope

In scope for v1:
- OpenAI API integration for chat completion (streaming) and embeddings
- One PDF knowledge base, ingested once via a script, plus a bundled demo corpus used when no PDF is present
- RAG pipeline: chunk the source, embed the chunks, store them in Qdrant, retrieve the most relevant chunks per question, and ground the answer in them
- Token-by-token streaming of the assistant reply to the widget
...
1. The operator runs the ingestion script once with the client's PDF. The script extracts text, splits it into chunks, embeds each chunk, and stores them in a Qdrant collection. If no PDF is supplied, the script (or the app on first start) seeds the same collection from the bundled demo corpus so RAG works immediately.
...
4. The backend embeds the question, searches Qdrant for the closest chunks, builds a prompt that includes only that retrieved context, calls OpenAI with streaming enabled, strips any em-dash characters defensively from each chunk, streams the reply to the widget, stores the completed exchange in SQLite.
5. The widget renders the reply token by token as it arrives.

## 4. RAG Pipeline

Ingestion (run once per PDF, or re-run if the client sends an updated PDF; also used to seed the demo corpus):
- Extract text from the PDF page by page (or read the bundled demo corpus when no PDF is given).
...
- Upsert into a Qdrant collection, storing the chunk text, source page number (or section for the demo corpus), and chunk index as payload alongside the vector.
- The collection is recreated fresh on each ingestion run, since v1 only supports a single source.
...
- As chunks stream back, replace any em-dash characters with a comma or period as a safeguard, even though the system prompt already forbids them, and forward each cleaned chunk to the client.
...
All request and response bodies are Pydantic models. PDF ingestion is a CLI script (`scripts/ingest_pdf.py`), not a public endpoint. A public upload route is unnecessary scope for a single, client-supplied PDF and avoids an extra attack surface on a small budget project.
...
- Payload per point: chunk text, source page number (or demo-corpus section), chunk index

## 7. Frontend

- A Next.js (App Router) app hosting the chat widget UI: a floating bubble that expands into a chat window, plus a small demo page for testing during development. Components use shadcn/ui and Tailwind; visual design follows the `frontend-design` skill so the widget does not read as a templated default.
- The widget consumes the streaming `/api/chat` response and renders tokens as they arrive, with a visible typing/streaming state.
...
- `TOP_K` (default: 4)
- `USE_DEMO_CORPUS` (default: true; when no PDF has been ingested, seed and answer from the bundled demo corpus)

## 9. Deployment Plan

- Backend: a `docker-compose.yml` with the FastAPI backend service. It connects to the existing Qdrant server through `QDRANT_URL`, so Qdrant is not part of this compose file. The SQLite file is persisted through a mounted volume.
...
- `README.md` documents the exact steps: environment setup, `docker compose up`, running the ingestion script once against the client's PDF (or relying on the demo corpus), and pointing the client's website at the deployed `embed.js`.
...
2. PDF-based Q&A functionality: satisfied by the RAG pipeline (section 4), demonstrable with or without a client PDF via the demo corpus.
...
- The hosting target for the backend: a VPS, a platform like Render or Railway, or something already in use by the client.
